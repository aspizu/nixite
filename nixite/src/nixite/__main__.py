...
from nixite.string_builder import StringBuilder
...
registry: Registry = {}
for pkg_name, pkg in msgspec.toml.decode(Path("registry.toml").read_bytes()).items():
...
def build(distro: Distro, selection: set[str], registry: Registry):
    ...
    script.iprintln(Path(f"{distro.lower()}.sh").read_text())
...
script = build(Distro.ARCH, {"bun", "ruff", "rustup", "zen"}, registry)
